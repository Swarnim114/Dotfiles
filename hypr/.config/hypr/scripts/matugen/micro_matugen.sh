...
jq -r '
  ...
  # Better visible cursor and selection:
  "color-link cursor-line \"\($surface_variant)\"\n" +
  "color-link cursor \"#ffffff,\($primary)\"\n" +
  "color-link selection \"#ffffff,\($outline)\"\n" +

  # Better gutter visibility:
...
if [[ $? -eq 0 ]]; then
    ...
    echo ""
    echo "To use this theme in micro, add to settings.json:"
